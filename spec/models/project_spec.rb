...
RSpec.describe Project, type: :model do
  ...
  describe "#archived?" do
    it "returns true if archived" do
      expect(Project.new(status: "archived").archived?).to eql(true)
    end

    it "returns false if status is not `archived`" do
      expect(Project.new(status: nil).archived?).to eql(false)
    end
    ...
    it "returns true if the parent is archived" do
      ...
      expect(sub_project1.archived?).to eql(true)
    end
    ...
    it "returns false if the parent is not archived" do
      ...
      expect(sub_project1.archived?).to eql(false)
    end    
  end
  ...
  describe "#toggle_archived!" do
    ...
    it "won't update a sub project" do
      ...
      sub_project1.reload
      expect(sub_project1.status).to eql(nil)

      expect(sub_project2.toggle_archived!).to eql(nil)
      sub_project2.reload
      expect(sub_project2.status).to eql("archived")
    end
  end
